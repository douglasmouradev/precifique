...
@php
    $accountTabs = [
        'profile' => __('app.account.tab_profile'),
        'security' => __('app.account.tab_security'),
        'billing' => __('app.account.tab_billing'),
        'integrations' => __('app.account.tab_integrations'),
    ];
    if ($isOwner) {
        $accountTabs['team'] = __('app.account.tab_team');
    }
@endphp

<div class="max-w-5xl">
<x-ui.tabs :labels="$accountTabs" default="profile">
    <x-slot:profile>
        <x-ui.card>
    ...
    <x-slot:team>
        <x-ui.card>
            ...
            @if($members->isEmpty())
            <x-ui.empty-state icon="products" :title="__('members.no_members')" class="border-0 shadow-none py-4" />
            @else
            <ul class="divide-y divide-slate-100">
