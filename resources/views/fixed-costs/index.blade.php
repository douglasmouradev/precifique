...
@section('title', __('costs.fixed.title'))
@section('breadcrumb') {{ __('costs.fixed.breadcrumb') }} @endsection

@section('content')
<x-ui.page-header :title="__('costs.fixed.page_title')" :subtitle="__('costs.fixed.total_active', ['amount' => number_format($total, 2, ',', '.')])">
    <x-slot:actions>
        <x-ui.button variant="outline" :href="route('tenant.variable-costs.index')">{{ __('costs.fixed.variable_costs_link') }}</x-ui.button>
    </x-slot:actions>
</x-ui.page-header>
...
<div class="grid lg:grid-cols-2 gap-6 animate-fade-in">
    <x-ui.card>
        <x-slot:header>
            <h2 class="ui-section-title mb-0">{{ __('costs.fixed.add_section') }}</h2>
        </x-slot:header>
        <form method="POST" action="{{ route('tenant.fixed-costs.store') }}" class="space-y-4">
            @csrf
            <div><label class="ui-label">{{ __('costs.common.name') }}</label><input name="name" placeholder="{{ __('costs.fixed.name_placeholder') }}" required class="ui-input"></div>
            <div><label class="ui-label">{{ __('costs.common.value_currency') }}</label><input name="amount" type="number" step="0.01" placeholder="{{ __('costs.common.amount_placeholder') }}" required class="ui-input"></div>
    ...
    <div class="space-y-4">
        ...
        <x-ui.empty-state
            icon="fixed-costs"
            :title="__('costs.fixed.empty_title')"
            :description="__('costs.fixed.empty_description')"
        />
        @endforelse
    </div>
</div>
